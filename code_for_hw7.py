...
class Linear(Module):
    ...
    def forward(self, A):
        ...
        return np.dot(self.W.T, A) + self.W0   # (n X m) . (m x b) = (n x b)
...
class Tanh(Module):  # Layer activation
    ...
    def backward(self, dLdA):  # Uses stored self.A
        ...
        return dAdZ * dLdA  # Your code: return dLdZ (?, b) = (n, b)


class ReLU(Module):  # Layer activation
    def forward(self, Z):
        self.A = np.maximum(0, Z)  # Your code: (?, b) = (n, b)
        return self.A

    def backward(self, dLdA):  # uses stored self.A
        return dLdA * (self.A != 0)  # Your code: return dLdZ (?, b) = (n, b)


class SoftMax(Module):  # Output activation
    def forward(self, Z):
        den = np.sum(np.exp(Z), axis=0)
        return np.exp(Z)/den    # Your code: (?, b) = (n, b)
...
class NLL(Module):  # Loss
    ...
    def backward(self):  # Use stored self.Ypred, self.Y
        return (self.Ypred - self.Y) # Your code (?, b) = (n, b)
...
class Sequential:
    ...
    def sgd(self, X, Y, iters=100, lrate=0.005):  # Train
        ...
        for it in range(iters):
            i = np.random.randint(N)  # Your code
            Xt = X[:, i:i+1]
            Yt = Y[:, i:i+1]
